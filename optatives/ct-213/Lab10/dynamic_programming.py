...
def policy_evaluation(grid_world, initial_value, policy, num_iterations=10000, epsilon=1.0e-5):
    ...
    rewards = np.zeros(dimensions)
    iteration = 0

    for i in range(dimensions[0]):
        for j in range(dimensions[1]):
            for action in range(NUM_ACTIONS):
                rewards[i, j] += policy[i, j, action]*grid_world.reward((i, j), action)

    while iteration < num_iterations:
        for i in range(dimensions[0]):
            for j in range(dimensions[1]):
                value[i, j] = rewards[i, j]
                for action in range(NUM_ACTIONS):
                    for next_state in grid_world.get_valid_sucessors((i, j), action):
                        transition_prob = grid_world.transition_probability((i, j), action, next_state)
                        next_state_value = initial_value[next_state[0], next_state[1]]
                        value[i, j] += grid_world.gamma*policy[i, j, action]*transition_prob*next_state_value

        convergence = np.amax(np.absolute(value - initial_value))
        ...
        for i in range(dimensions[0]):
            for j in range(dimensions[1]):
                ...
                for action in range(NUM_ACTIONS):
                    ...
                    for next_state in grid_world.get_valid_sucessors((i, j), action):
                        transition_prob = grid_world.transition_probability((i, j), action, next_state)
                        pss += grid_world.gamma*transition_prob*initial_value[next_state[0], next_state[1]]

                    if value[i, j] < grid_world.reward((i, j), action) + pss:
                        value[i, j] = grid_world.reward((i, j), action) + pss
...
def policy_iteration(grid_world, initial_value, initial_policy, evaluations_per_policy=3, num_iterations=10000,
    ...
    while iteration < num_iterations:
        value = policy_evaluation(grid_world, initial_value, initial_policy, num_iterations=evaluations_per_policy)

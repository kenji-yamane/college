...
class NeuralNetwork:
    ...
    def compute_gradient_back_propagation(self, inputs, expected_outputs):
        ...
        for i in range(len(inputs)):
            ...
            delta2 += a[-1] - expected_outputs[i]
            weights_gradient[2] += delta2*(a[1].transpose())
            biases_gradient[2] += delta2
            delta1 = np.zeros((self.num_hiddens, 1))
            delta1 += ((delta2.transpose())*self.weights[2]).transpose()
            delta1 *= sigmoid_derivative(z[1])
            weights_gradient[1] += delta1*(a[0].transpose())
            biases_gradient[1] += delta1
        weights_gradient[1] /= len(inputs)
        weights_gradient[2] /= len(inputs)
        biases_gradient[1] /= len(inputs)
        biases_gradient[2] /= len(inputs)

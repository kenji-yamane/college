...
fig_format = 'pdf'  # 'svg' (Word), 'eps' (Latex), 'png' (best compatibility/worst quality)
...
def benchmark_algorithm(num_trials, num_iterations, algorithm, function, hyperparams):
    ...
    for k in range(num_trials):
        ...
        if algorithm == 'ses':
            hyperparams.C0 = np.identity(2)
            es = SimpleEvolutionStrategy(m0, hyperparams.C0, hyperparams.mu, hyperparams.population_size)
        else:
